...
const renderResult = () => {
    ...
    result.innerHTML = `
		...
		<div class="score__card" >
			<div class="score__card__greet"><h2>Congratulations!</h2></div>
			<div class="final__result">
				<h4>You won the game!</h3>
				<p>Stars Awarded: ${star}</p>
				<p>Moves Done: ${moves}</p>
...
const openCardOnClick = () => {
    cards.addEventListener('click', e => {
        ...
        //If open card clicked do nothing
        if (card == "") return;
        ...
        moves++;
        updateMoves();
        //Hide cards when 2 different cards are opened
        if (prevCard != "") {
            if (prevCard.getAttribute('data-item') === card.getAttribute('data-item')) {
                ...
                //Copying of prev card to temp is important cause the following function is called after 0.5s delay, so prevCard will be set to "" after 0.5s. But if a card is clicked before 0.5s prevCard will not yet be set to "" which may give some error
                let temp = prevCard;
                prevCard = "";
                setTimeout(() => {
